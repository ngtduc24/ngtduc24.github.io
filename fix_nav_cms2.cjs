...
// Try replacing using a more robust split
const parts = content.split('{/* SUB-VIEW 3: NAVIGATION MENU BUILDER PANEL */}');
if (parts.length > 1) {
  // Find where this block ends... it should be near the end of the file, just before the helper function
  const endParts = parts[1].split('// Helper safe field reader');
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    ...
                    <select
                      ...
                      onChange={(e) => setEditingNav({ ...editingNav, deviceType: e.target.value })}
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl py-2 px-3 text-xs focus:outline-none focus:border-emerald-500"
                    >
                  ...
                  <div className="space-y-1">
                    ...
                    <select
                      ...
                      onChange={(e) => setEditingNav({ ...editingNav, target: e.target.value })}
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl py-2 px-3 text-xs focus:outline-none focus:border-emerald-500"
                    >
...
`;
  ...
  content = parts[0] + navCode + '\n  // Helper safe field reader' + endParts[1];
}
