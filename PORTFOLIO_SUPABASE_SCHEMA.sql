...
begin
  foreach table_name in array array[
    'portfolio_settings','portfolio_education','portfolio_experience','portfolio_skills',
    'portfolio_projects','portfolio_courses','portfolio_course_chapters','portfolio_course_lessons',
    'portfolio_course_students','portfolio_research','portfolio_lectures','media_library'
  ] loop
    ...
    execute format('drop policy if exists %I_app_write on public.%I', table_name, table_name);
    execute format('create policy %I_app_write on public.%I for all to anon, authenticated using (true) with check (true)', table_name, table_name);
  end loop;
end $$;
...
grant select, insert, update, delete on table
  ...
  public.portfolio_courses,
  public.portfolio_course_chapters,
  public.portfolio_course_lessons,
  public.portfolio_course_students,
  public.portfolio_research,
  public.portfolio_lectures,
  public.media_library
to anon, authenticated;

-- Realtime cho các phần cần cập nhật ngay trên trang Portfolio.
do $$
begin
...
-- LƯU Ý BẢO MẬT:
-- Ứng dụng hiện dùng hệ thống đăng nhập riêng và gọi Supabase bằng anon key,
-- nên policy ghi tạm thời cho phép anon/authenticated để tương thích.
-- Khi chuyển đăng nhập sang Supabase Auth, hãy đổi policy *_app_write thành:
