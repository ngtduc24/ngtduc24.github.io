...
function buildMenuFromJson(menus) {
    ...
    menus.forEach(item => {
        ...
        // Đánh dấu active nếu là trang chủ
        if (item.slug === 'home' && window.location.pathname.includes('index.html')) {
        ...
        if (item.children && item.children.length > 0) {
            ...
            li.onmouseenter = function() { dropdown.style.display = 'block'; };
            li.onmouseleave = function() { dropdown.style.display = 'none'; };
        }
