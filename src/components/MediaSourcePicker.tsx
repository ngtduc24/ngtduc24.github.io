...
export default function MediaSourcePicker({
  ...
  const [uploadType, setUploadType] = useState<CloudinaryResourceType>(resourceType === 'auto' ? 'image' : resourceType);
  ...
  const wantedType = resourceType === 'auto'
    ? (accept.includes('image') && accept.includes('video') ? 'auto' : accept.includes('image') ? 'image' : accept.includes('video') ? 'video' : '')
    : resourceType;
  ...
  const uploadFiles = async (files: File[]) => {
    ...
    setError('');
    try {
      const urls = await Promise.all(files.map(file => uploadMediaToCloudinary(file, { resourceType: uploadType, folder, category })));
  ...
  return (
    <>
      <button
        ...
        className={className || `inline-flex items-center justify-center gap-2 rounded-full bg-brand px-3 py-2.5 text-xs font-bold text-white transition-colors hover:bg-brand-hover disabled:bg-slate-400 ${compact ? 'h-9 w-9 p-0' : ''}`}
        title={label}
      >
        {uploading ? (
          ...
          <div className="flex max-h-[88vh] w-full max-w-4xl flex-col overflow-hidden rounded-3xl bg-white shadow-2xl">
            ...
            {mode === 'library' && (
              <div className="flex min-h-0 flex-1 flex-col p-5">
                ...
                {multiple && <div className="mt-4 flex items-center justify-between border-t border-slate-100 pt-4"><span className="text-xs font-semibold text-slate-500">Đã chọn {selected.length} tệp</span><button type="button" disabled={!selected.length} onClick={confirmMultiple} className="rounded-full bg-brand px-4 py-2.5 text-xs font-bold text-white disabled:bg-slate-300">Dùng các tệp đã chọn</button></div>}
              </div>
            )}
              ...
              <div className="p-5 sm:p-7">
                ...
                {resourceType === 'auto' && (
                  <div className="flex gap-2 mb-4">
                     <button type="button" onClick={() => {}} className="px-4 py-2 bg-brand text-white rounded-lg text-xs font-bold">Hình ảnh</button>
                     <button type="button" onClick={() => {}} className="px-4 py-2 bg-slate-100 text-slate-700 rounded-lg text-xs font-bold">Video</button>
                  </div>
                )}
                ...
                {resourceType === 'auto' && (
                  <div className="flex gap-2 mb-4">
                     <button type="button" onClick={() => setUploadType('image')} className={`px-4 py-2 rounded-lg text-xs font-bold ${uploadType === 'image' ? 'bg-brand text-white' : 'bg-slate-100 text-slate-700'}`}>Hình ảnh</button>
                     <button type="button" onClick={() => setUploadType('video')} className={`px-4 py-2 rounded-lg text-xs font-bold ${uploadType === 'video' ? 'bg-brand text-white' : 'bg-slate-100 text-slate-700'}`}>Video</button>
                  </div>
                )}
                ...
                <div className="flex min-h-60 flex-col items-center justify-center rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50 p-6 text-center" onDragOver={event => event.preventDefault()} onDrop={event => { event.preventDefault(); void uploadFiles(Array.from(event.dataTransfer.files)); }}>
                  ...
                  <button type="button" disabled={uploading} onClick={() => inputRef.current?.click()} className="mt-5 rounded-full bg-brand px-4 py-2.5 text-xs font-bold text-white disabled:bg-slate-400">Chọn tệp mới</button>
                  <input ref={inputRef} type="file" accept={uploadType === 'video' ? 'video/*' : 'image/*'} multiple={multiple} className="hidden" onChange={event => void uploadFiles(event.target.files ? Array.from(event.target.files) : [])} />
                </div>
              </div>
            )}
