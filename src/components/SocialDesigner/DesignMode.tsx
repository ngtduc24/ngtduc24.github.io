    ...
    <div className="flex flex-col w-full h-full bg-slate-100">
      ...
      <div className="bg-white border-b border-slate-200 px-3 sm:px-4 py-2 sm:py-2.5 flex items-center justify-between gap-3 shrink-0 shadow-xs z-20">
        <div className="flex items-center gap-2 sm:gap-3 min-w-0">
          ...
          <select 
            className="bg-slate-50 border border-slate-200 rounded-lg px-2.5 sm:px-3 py-1.5 text-xs sm:text-sm font-semibold text-slate-700 outline-none focus:border-brand max-w-[170px] sm:max-w-[260px] truncate"
        ...
        <div className="flex items-center gap-2 ml-auto">
          ...
          <div className="bg-slate-100 p-0.5 sm:p-1 rounded-lg flex border border-slate-200/80">
            <button 
              ...
              className={`px-2.5 sm:px-3 py-1 rounded-md text-xs font-semibold transition ${designTab === 'basic' ? 'bg-white shadow-2xs text-brand-hover font-bold' : 'text-slate-500 hover:text-slate-800'}`}
            >
              Nội dung
            </button>
            <button 
              ...
              className={`px-2.5 sm:px-3 py-1 rounded-md text-xs font-semibold transition ${designTab === 'advanced' ? 'bg-white shadow-2xs text-brand-hover font-bold' : 'text-slate-500 hover:text-slate-800'}`}
            >
          ...
          <button 
            ...
            className="bg-brand text-white px-3 sm:px-4 py-1.5 rounded-lg text-xs sm:text-sm font-semibold hover:bg-brand-hover disabled:opacity-60 flex items-center gap-1.5 shadow-xs transition active:scale-95 shrink-0"
          >
            {isExporting ? (
              <>
        ...
        <div className="order-2 lg:order-1 flex-1 lg:flex-none w-full lg:w-[460px] xl:w-[490px] bg-white lg:border-r border-slate-200 flex flex-col overflow-y-auto min-h-0">
          {designTab === 'basic' ? (
            <div className="p-4 space-y-4">
              ...
              <div className="bg-gradient-to-r from-brand-light/90 to-slate-50 border border-brand-light rounded-xl p-3 space-y-2.5 shadow-2xs">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-brand-hover flex items-center gap-1.5">
                    <Bookmark className="w-3.5 h-3.5 text-brand" /> Vị trí chữ & Preset
                  </span>
                  <button 
                    type="button"
                    onClick={() => {
                    ...
                    className="text-[11px] font-semibold text-brand-hover hover:text-brand-hover bg-white border border-brand hover:border-brand px-2 py-1 rounded-lg flex items-center gap-1 shadow-2xs transition"
                  >
                    <Plus className="w-3 h-3 text-brand" /> Lưu Preset
                  </button>
                </div>
                ...
                <div className="relative">
                  <select 
                    ...
                    className="w-full bg-white border border-brand text-slate-700 text-xs font-medium rounded-lg px-2.5 py-1.5 outline-none focus:border-brand focus:ring-1 focus:ring-brand"
                  >
                ...
                {selectedPresetId && (
                  <div className="flex items-center justify-between text-[11px] text-slate-500 pt-0.5">
                    <span className="truncate max-w-[230px]">
                      Đang dùng: <strong className="text-brand-hover font-semibold">{presets.find(p => p.id === selectedPresetId)?.name}</strong>
                    </span>
                ...
                {presetSuccessMsg && (
                  <div className="text-[11px] font-semibold text-brand-hover bg-brand-light border border-brand px-2.5 py-1 rounded-md flex items-center gap-1.5 animate-fadeIn">
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-600 flex items-center gap-1.5">
                  <ImageIcon className="w-3.5 h-3.5 text-brand" /> Ảnh chính
                </label>
                <div className="relative group rounded-xl overflow-hidden border-2 border-dashed border-slate-300 hover:border-brand h-28 flex items-center justify-center bg-slate-50">
                  {contentImage ? (
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-700 flex items-center gap-1.5">
                  <Type className="w-3.5 h-3.5 text-brand" /> Tiêu đề tin tức
                </label>
                <textarea 
                  value={contentTitle}
                  onChange={e => setContentTitle(e.target.value)}
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl p-2.5 text-sm focus:outline-none focus:border-brand min-h-[70px] resize-none font-medium text-slate-800"
                    ...
                    <div>
                      <div className="flex items-center justify-between text-xs font-medium text-slate-600 mb-1">
                        <span className="flex items-center gap-1 font-semibold text-slate-700">
                          <Sliders className="w-3 h-3 text-brand" /> Vị trí dọc (Y):
                        </span>
                        <span className="font-bold text-brand-hover bg-white px-2 py-0.5 rounded border border-brand-light text-[11px] shadow-2xs">
                      ...
                      <input 
                        ...
                        className="w-full accent-brand cursor-pointer h-1.5 bg-slate-200 rounded-lg appearance-none"
                      />
                      ...
                      <div className="grid grid-cols-3 gap-1.5 mt-2">
                        <button
                          ...
                          className={`text-[10px] py-1 px-1 rounded font-medium border text-center transition ${Math.round(titleLayer.y) === 25 ? 'bg-brand-light border-brand text-brand-hover font-bold' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-100'}`}
                        >
                          Trên ảnh (25%)
                        </button>
                        <button
                          ...
                          className={`text-[10px] py-1 px-1 rounded font-medium border text-center transition ${Math.round(titleLayer.y) === 50 ? 'bg-brand-light border-brand text-brand-hover font-bold' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-100'}`}
                        >
                          Giữa (50%)
                        </button>
                        <button
                          ...
                          className={`text-[10px] py-1 px-1 rounded font-medium border text-center transition ${Math.round(titleLayer.y) === 72 ? 'bg-brand-light border-brand text-brand-hover font-bold' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-100'}`}
                        >
                          Đáy thẻ (72%)
                        </button>
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 pt-1 border-t border-slate-200/70">
                      <div>
                        <div className="flex items-center justify-between text-[11px] font-semibold text-slate-600 mb-1">
                          <span>Độ rộng:</span>
                          <span className="text-brand font-bold bg-white px-1.5 py-0.5 rounded border border-slate-200 text-[10px]">
                      ...
                      <div className="flex items-center gap-2">
                        {[
                          ...
                          <button
                            ...
                            className={`w-6 h-6 rounded-full border shadow-2xs transition transform hover:scale-110 flex items-center justify-center ${titleLayer.color?.toLowerCase() === c.color.toLowerCase() ? 'ring-2 ring-brand ring-offset-1 scale-105' : 'border-slate-300'}`}
                        ...
                        {/* Custom Color Picker */}
                        <label className="w-6 h-6 rounded-full border border-slate-300 cursor-pointer overflow-hidden flex items-center justify-center relative hover:scale-110 transition shadow-2xs bg-gradient-to-tr from-brand to-pink-200" title="Chọn màu tùy ý">
                          <input 
                            type="color" 
                      ...
                      <div>
                        ...
                        <div className="flex items-center bg-white border border-slate-200 rounded p-0.5 shadow-2xs">
                          <button
                            ...
                            className={`flex-1 py-1 flex items-center justify-center rounded ${titleLayer.textAlign === 'left' || !titleLayer.textAlign ? 'bg-brand-light text-brand font-bold' : 'text-slate-400 hover:text-slate-600'}`}
                            title="Căn trái"
                          >
                          ...
                          <button
                            ...
                            className={`flex-1 py-1 flex items-center justify-center rounded ${titleLayer.textAlign === 'center' ? 'bg-brand-light text-brand font-bold' : 'text-slate-400 hover:text-slate-600'}`}
                            title="Căn giữa"
                          >
                          ...
                          <button
                            ...
                            className={`flex-1 py-1 flex items-center justify-center rounded ${titleLayer.textAlign === 'right' ? 'bg-brand-light text-brand font-bold' : 'text-slate-400 hover:text-slate-600'}`}
                            title="Căn phải"
                          >
              ...
              <div className="space-y-2">
                ...
                <textarea 
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl p-2.5 text-sm focus:outline-none focus:border-brand min-h-[80px] resize-none text-slate-700 leading-relaxed"
                ...
                {descLayer && (
                  <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 space-y-3">
                    ...
                    <div>
                      <div className="flex items-center justify-between text-xs font-medium text-slate-600 mb-1">
                        <span className="flex items-center gap-1 font-semibold text-slate-700">
                          <Sliders className="w-3 h-3 text-brand" /> Vị trí dọc mô tả (Y):
                        </span>
                        <span className="font-bold text-brand-hover bg-white px-2 py-0.5 rounded border border-brand-light text-[11px] shadow-2xs">
                      ...
                      <div>
                        ...
                        <div className="flex items-center bg-white border border-slate-200 rounded p-0.5 shadow-2xs">
                          <button
                            ...
                            className={`flex-1 py-1 flex items-center justify-center rounded ${descLayer.textAlign === 'left' || !descLayer.textAlign ? 'bg-brand-light text-brand font-bold' : 'text-slate-400 hover:text-slate-600'}`}
                            title="Căn trái"
                          >
                            <AlignLeft className="w-3.5 h-3.5" />
                          </button>
                          <button
                            ...
                            className={`flex-1 py-1 flex items-center justify-center rounded ${descLayer.textAlign === 'center' ? 'bg-brand-light text-brand font-bold' : 'text-slate-400 hover:text-slate-600'}`}
                            title="Căn giữa"
                          >
                            <AlignCenter className="w-3.5 h-3.5" />
                          </button>
                          <button
                            ...
                            className={`flex-1 py-1 flex items-center justify-center rounded ${descLayer.textAlign === 'right' ? 'bg-brand-light text-brand font-bold' : 'text-slate-400 hover:text-slate-600'}`}
                            title="Căn phải"
                          >
                            <AlignRight className="w-3.5 h-3.5" />
                          </button>
                      ...
                      <div className="flex items-center gap-1.5">
                        {[
                          ...
                          <button
                            ...
                            className={`w-5 h-5 rounded-full border shadow-2xs transition transform hover:scale-110 flex items-center justify-center ${descLayer.color?.toLowerCase() === c.color.toLowerCase() ? 'ring-2 ring-brand ring-offset-1 scale-105' : 'border-slate-300'}`}
                            style={{ backgroundColor: c.color }}
                            title={c.name}
                          >
               ...
               {activeTemplateConfig && activeTemplateConfig.layers.map(layer => (
                 <div key={layer.id} className="mb-4 bg-slate-50 border border-slate-200 rounded-xl p-3">
                   <div className="flex items-center gap-2 mb-3">
                     {layer.type === 'image' ? <ImageIcon className="w-4 h-4 text-brand" /> : <TypeIcon className="w-4 h-4 text-brand" />}
          ...
          <div className="absolute top-2.5 right-2.5 sm:top-4 sm:right-4 z-20 flex items-center gap-2 bg-white/90 backdrop-blur-md px-2.5 py-1.5 rounded-xl border border-slate-200/80 shadow-xs">
            ...
            <div className="hidden lg:flex items-center gap-1 text-[11px] font-semibold text-slate-600 border-r border-slate-200 pr-2 mr-1">
              ...
              <button
                ...
                className={`px-2 py-0.5 rounded transition ${desktopPreviewSize === 380 ? 'bg-brand-light text-brand-hover font-bold border border-brand shadow-2xs' : 'hover:bg-slate-100'}`}
              ...
              <button
                ...
                className={`px-2 py-0.5 rounded transition ${desktopPreviewSize === 460 ? 'bg-brand-light text-brand-hover font-bold border border-brand shadow-2xs' : 'hover:bg-slate-100'}`}
              ...
              <button
                ...
                className={`px-2 py-0.5 rounded transition ${desktopPreviewSize === 560 ? 'bg-brand-light text-brand-hover font-bold border border-brand shadow-2xs' : 'hover:bg-slate-100'}`}
            ...
            <button
              ...
              onClick={handleExportImage}
              disabled={isExporting}
              className="flex items-center gap-1 text-xs font-bold text-white bg-brand hover:bg-brand-hover px-2.5 py-1 rounded-lg shadow-2xs transition active:scale-95 disabled:opacity-60"
            >
            ...
            <form onSubmit={handleSavePreset} className="p-5 space-y-4">
              <div>
                ...
                <input 
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2 text-sm text-slate-800 focus:outline-none focus:border-brand focus:bg-white transition"
                  autoFocus
                />
              </div>
              ...
              <div className="bg-slate-50 border border-slate-200/80 rounded-xl p-3 space-y-1.5 text-xs text-slate-600">
                <div className="font-bold text-slate-700 mb-1 flex items-center gap-1">
                  <Sliders className="w-3 h-3 text-brand" /> Thông số sẽ lưu:
                </div>
              ...
              <div className="flex items-center justify-end gap-2.5 pt-2">
                ...
                <button
                  ...
                  className="px-5 py-2 text-xs font-bold text-white bg-brand hover:bg-brand-hover disabled:opacity-50 rounded-xl shadow-sm transition flex items-center gap-1.5"
                >
                  {savingPreset ? (
                    <span>Đang lưu...</span>
                  ) : (
          ...
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-100 w-full max-w-md overflow-hidden animate-fadeIn">
            ...
            <div className="flex items-center justify-between p-4 sm:p-5 border-b border-slate-100 bg-slate-50/50">
              <div className="flex items-center gap-3">
                {exportResult ? (
                  <div className="w-10 h-10 rounded-xl bg-brand-light border border-brand text-brand flex items-center justify-center shrink-0 shadow-2xs">
