...
import { auth, authAdminClient } from '../lib/firebase';
...
export default function UserManagement({ currentUser, users, onSaveUser, onDeleteUser, settings, onRefreshSettings }: UserManagementProps) {
  ...
  const handleEditUserSubmit = async (e: React.FormEvent) => {
    ...
    const updatedUser: UserAccount = {
      ...
      permissions: editRole === 'admin'
        ? ['dashboard', 'calculator', 'scientific_journals', 'tasks', 'qualitative_analysis', 'quantitative_analysis', 'media_library', 'portfolio_cms', 'settings', 'notifications', 'users']
        : editPermissions,
  ...
  const handleAddUser = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Create the user in Firebase Auth first using the secondary client
      let authUserId = '';
      try {
        const userCred = await createUserWithEmailAndPassword(authAdminClient, newEmail.trim(), newPassword.trim());
        authUserId = userCred.user.uid;
      } catch (authErr: any) {
        ...
        if (authErr.code === 'auth/email-already-in-use') {
          ...
          setFormError('Mật khẩu quá yếu. Vui lòng chọn mật khẩu mạnh hơn (ít nhất 6 ký tự).');
        } else {
          setFormError('Lỗi thiết lập Firebase Auth: ' + (authErr.message || authErr));
        }
        return;
      }
      ...
      const newUser: UserAccount = {
        ...
        email: newEmail.trim(),
        role: newRole,
        permissions: newRole === 'admin' ? ['dashboard', 'calculator', 'scientific_journals', 'tasks', 'qualitative_analysis', 'quantitative_analysis', 'media_library', 'portfolio_cms', 'settings', 'notifications', 'users'] : newPermissions,
        ...
        <div className="bg-white rounded-2xl border border-brand/20 p-6 shadow-md shadow-brand/5 animate-fadeIn">
          ...
          <form onSubmit={handleAddUser} className="space-y-4">
            ...
            <div className="flex justify-end gap-2.5">
              ...
              <button 
                type="submit"
                className="px-5 py-2.5 rounded-xl bg-brand hover:bg-brand-hover text-white font-bold text-xs shadow-md shadow-brand/10 cursor-pointer transition-all"
              >
                Lưu tài khoản
              </button>
            </div>
                  ...
                  <tr key={user.id} className="hover:bg-slate-50/40 transition-colors">
                    ...
                    <td className="py-3 px-4">
                      <button 
                        ...
                        disabled={isMe}
                        title={isMe ? 'Bạn không thể tự thay đổi vai trò của mình' : 'Bấm để chuyển đổi vai trò'}
