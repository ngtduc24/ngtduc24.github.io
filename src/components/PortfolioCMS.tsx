...
import ResearchLecturesNavCMS from './cms/ResearchLecturesNavCMS';
...
type PortfolioDivision = 'profile' | 'content' | 'projects' | 'courses' | 'research' | 'lectures' | 'navigation';
...
const DIVISIONS: Array<{
  ...
  },
  {
    id: 'lectures',
    title: 'Bài giảng',
    description: 'Danh sách bài giảng, PDF, slide và học liệu',
    icon: BookOpen
  },
  {
    id: 'navigation',
...
export default function PortfolioCMS() {
  const [activeDivision, setActiveDivision] = useState<PortfolioDivision>('profile');
  ...
  const [overviewStats, setOverviewStats] = useState([
    ...
    { label: 'Bài giảng', value: 0, detail: '0 công khai', icon: BookOpen }
  ]);

  useEffect(() => {
    Promise.all([getPortfolioProjects(), getPortfolioCourses(), getPortfolioResearch(), getPortfolioLectures()]).then(([projects, courses, research, lectures]) => setOverviewStats([
      ...
      { label: 'Bài giảng', value: lectures.length, detail: `${lectures.filter(item => item.status === 'published').length} công khai`, icon: BookOpen }
  ...
  const isContentListDivision = ['projects', 'courses', 'research', 'lectures'].includes(activeDivision);
  ...
  return (
    <div className="space-y-6 animate-fadeIn pb-12 text-slate-800">
      ...
      <section className={isContentListDivision ? 'min-h-[440px]' : 'min-h-[440px] rounded-3xl bg-white p-5 shadow-sm sm:p-6'}>
        ...
        {activeDivision === 'research' && <ResearchLecturesNavCMS initialSubTab="research" showSubTabs={false} />}
        {activeDivision === 'lectures' && <ResearchLecturesNavCMS initialSubTab="lectures" showSubTabs={false} />}
