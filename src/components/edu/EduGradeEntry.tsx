      ...
      {step === 1 && (
        <div className="space-y-4">
          <div className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
            <label className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Chọn lớp trong file .fg</label>
            <select
              value={fgClassIdx}
              onChange={e => setFgClassIdx(Number(e.target.value))}
              className="mt-1.5 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm font-semibold text-slate-800 outline-none focus:border-brand focus:bg-white"
            >
              <option value={-1}>— Chọn lớp —</option>
              {fgClasses.map(c => (
                <option key={c.index} value={c.index}>{c.className} · {c.subject} · {c.students.length} SV · {c.components.length} cột điểm</option>
              ))}
            </select>
          </div>
          {fgClass && (
