      ...
      <div className="space-y-4">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {lessons.map(l => (
              <div key={l.id} className="flex flex-col overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm">
                <div className="h-28 bg-slate-100 bg-cover bg-center" style={l.cover_url ? { backgroundImage: `url(${l.cover_url})` } : undefined}>
                  {!l.cover_url && <div className="flex h-full items-center justify-center text-slate-300"><BookOpen className="h-8 w-8" /></div>}
                </div>
                <div className="flex flex-1 flex-col p-4">
                  ...
                  <div className="mt-3 flex gap-1.5 border-t border-slate-50 pt-3">
                    <a href={`${window.location.origin}${window.location.pathname}?elesson=${l.share_token}`} target="_blank" rel="noreferrer" className="flex-1 rounded-lg bg-slate-100 px-2 py-1.5 text-center text-[10px] font-bold text-slate-600 hover:bg-slate-200">Xem</a>
