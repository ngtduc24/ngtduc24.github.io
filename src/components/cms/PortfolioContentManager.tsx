...
import { Award, BookOpen, Edit3, FileText, FolderGit2, GraduationCap, Newspaper, Plus, Search, Trash2 } from 'lucide-react';
import { deletePortfolioPost, getPortfolioPosts, savePortfolioPost } from '../../lib/portfolioData';
...
import ResearchLecturesNavCMS from './ResearchLecturesNavCMS';

type SelectedContentType = 'article' | 'project' | 'course' | 'research' | 'lecture' | null;
...
const contentTypes = [
  ...
  { id: 'lecture', label: 'Bài giảng & tài liệu', description: 'Cây học liệu gồm PDF, slide, video và tệp thực hành.', icon: BookOpen }
] as const;
...
export default function PortfolioContentManager() {
  ...
  useEffect(() => { getPortfolioPosts().then(setPosts); }, []);
  ...
  return (
    <div className="space-y-6">
      {message && <div className="fixed bottom-6 right-6 z-50 rounded-2xl bg-emerald-600 px-5 py-3 text-xs font-bold text-white shadow-xl">{message}</div>}
      ...
      selectedType === 'research' ? <div key={`research-${editorVersion}`}><ResearchLecturesNavCMS initialSubTab="research" createOnMount showSubTabs={false} /></div> :
      selectedType === 'lecture' ? <div key={`lecture-${editorVersion}`}><ResearchLecturesNavCMS initialSubTab="lectures" createOnMount showSubTabs={false} /></div> :
      editing ? (
        <form onSubmit={save} className="space-y-5 rounded-2xl bg-white">
          ...
          <div className="grid gap-5 lg:grid-cols-2">
            ...
            <div className="space-y-4">
              <div className="grid gap-3 sm:grid-cols-2"><label className="space-y-1"><span className="text-[10px] font-black uppercase tracking-wider text-slate-500">Chuyên mục</span><input value={editing.category} onChange={event => setEditing({ ...editing, category: event.target.value })} className={fieldClass} /></label><label className="space-y-1"><span className="text-[10px] font-black uppercase tracking-wider text-slate-500">Tác giả</span><input value={editing.author} onChange={event => setEditing({ ...editing, author: event.target.value })} className={fieldClass} /></label></div>
