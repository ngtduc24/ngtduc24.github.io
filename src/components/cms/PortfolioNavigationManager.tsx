...
import {
  ArrowDown, ArrowUp, BookOpen, Boxes, BriefcaseBusiness, Check, ChevronDown, ChevronRight,
  Compass, Edit3, ExternalLink, Eye, EyeOff, FileText, FolderGit2, GraduationCap, GripVertical,
  Globe2, Home, Image, Layout, Library, Link as LinkIcon, LockKeyhole, Mail, MapPin, Monitor,
  Newspaper, Phone, Plus, Presentation, RotateCcw, Smartphone, Sparkles, Trash2, Undo2, User,
  Video, XCircle
} from 'lucide-react';
import { PortfolioCourse, PortfolioNavigation, PortfolioPost, PortfolioProject } from '../portfolioTypes';
import {
...
export default function PortfolioNavigationManager() {
  ...
  useEffect(() => {
    Promise.all([getPortfolioNavigation(), getPortfolioProjects(), getPortfolioCourses(), getPortfolioPosts()])
      .then(([data, projectData, courseData, postData]) => {
      ...
      .catch(() => setError('Không thể tải cấu hình menu.'))
  ...
  const persist = async (nextItems: PortfolioNavigation[], text: string, remember = true) => {
    ...
    notify(synced ? text : `${text} Dữ liệu đã lưu trên máy nhưng chưa đồng bộ máy chủ.`);
  };
  ...
  const saveEditing = async (event: React.FormEvent) => {
    ...
    if (!editing) return;
    if (editing.locked) {
      setError('Menu cuộn mặc định là thành phần hệ thống và không thể chỉnh sửa.');
      return;
    }
    ...
    const normalizedEditing = {
      ...
      deviceVisibility: editing.deviceVisibility || 'all'
    };
    const exists = items.some(item => item.id === editing.id);
  ...
  const deleteItem = async (item: PortfolioNavigation) => {
    if (item.locked || item.kind === 'scroll') return setError('Menu cuộn mặc định không thể xóa.');
    const children = items.filter(child => child.parentId === item.id);
    const note = children.length ? ` Mục này có ${children.length} menu con; các menu con sẽ được chuyển lên cấp chính.` : '';
    if (!window.confirm(`Xóa mục “${item.label}”?${note}`)) return;
    const next = items
  ...
  const toggleVisibility = (item: PortfolioNavigation) =>
    item.locked ? setError('Menu cuộn mặc định không thể thay đổi.') : persist(items.map(value => value.id === item.id ? { ...value, visible: !value.visible } : value), item.visible ? 'Đã ẩn mục menu.' : 'Đã hiển thị mục menu.');

  const moveWithinLevel = (item: PortfolioNavigation, direction: 'up' | 'down') => {
    if (item.locked) return setError('Menu cuộn mặc định được cố định thứ tự.');
  ...
  const dropOnItem = (target: PortfolioNavigation) => {
    ...
    if (dragged.locked || target.locked) {
      setError('Không thể kéo thả menu cuộn mặc định.');
      return setDraggedId(null);
    }
    if (dragged.parentId !== target.parentId) {
      setError('Chỉ có thể kéo thả trong cùng một cấp menu. Để đổi cấp, hãy chỉnh trường Menu cha.');
      return setDraggedId(null);
    }
  ...
  const resetDefaults = async () => {
    if (!window.confirm('Khôi phục toàn bộ menu mặc định? Bạn có thể dùng Hoàn tác ngay sau thao tác này.')) return;
  ...
  return (
    <div className="space-y-6">
      {(message || error) && (
        <div className={`fixed bottom-6 right-6 z-50 flex max-w-sm items-start gap-2 rounded-2xl px-5 py-3 text-xs font-semibold text-white shadow-xl ${error ? 'bg-rose-600' : 'bg-emerald-600'}`}>
          {error ? <XCircle className="h-4 w-4 shrink-0" /> : <Check className="h-4 w-4 shrink-0" />}
        ...
        <section className="space-y-4 rounded-2xl bg-slate-50 p-5 sm:p-6 xl:order-2 xl:col-span-7">
          ...
          <div className="space-y-2">
            ...
            {displayItems.map(item => {
              ...
              return (
                <article key={item.id} draggable={!item.locked} onDragStart={() => !item.locked && setDraggedId(item.id)} onDragEnd={() => setDraggedId(null)} onDragOver={event => event.preventDefault()} onDrop={() => dropOnItem(item)} className={`flex items-center justify-between gap-3 rounded-xl bg-white p-3 shadow-xs transition ${item.parentId ? 'ml-5' : ''} ${draggedId === item.id ? 'opacity-50 ring-2 ring-brand/30' : ''}`}>
                  <div className="flex min-w-0 items-center gap-2.5">
                    {item.locked ? <LockKeyhole className="h-4 w-4 shrink-0 text-slate-300" /> : <GripVertical className="h-4 w-4 shrink-0 cursor-grab text-slate-300" />}
                  ...
                  <div className="flex shrink-0 items-center gap-1">
                    <button type="button" aria-label="Di chuyển lên" disabled={item.locked || siblingIndex === 0} onClick={() => moveWithinLevel(item, 'up')} className="rounded-lg bg-slate-50 p-1.5 disabled:opacity-25"><ArrowUp className="h-3.5 w-3.5" /></button>
                    <button type="button" aria-label="Di chuyển xuống" disabled={item.locked || siblingIndex === siblings.length - 1} onClick={() => moveWithinLevel(item, 'down')} className="rounded-lg bg-slate-50 p-1.5 disabled:opacity-25"><ArrowDown className="h-3.5 w-3.5" /></button>
                    <button type="button" aria-label={item.visible ? 'Ẩn mục menu' : 'Hiển thị mục menu'} disabled={item.locked} onClick={() => toggleVisibility(item)} className="rounded-lg bg-slate-50 p-1.5 disabled:opacity-25">{item.visible ? <Eye className="h-3.5 w-3.5" /> : <EyeOff className="h-3.5 w-3.5" />}</button>
                    <button type="button" aria-label="Chỉnh sửa" disabled={item.locked} onClick={() => { setError(''); setEditing({ ...item }); }} className="rounded-lg bg-slate-50 p-1.5 text-brand disabled:text-slate-300"><Edit3 className="h-3.5 w-3.5" /></button>
                    <button type="button" aria-label="Xóa mục menu" disabled={item.locked} onClick={() => deleteItem(item)} className="rounded-lg bg-rose-50 p-1.5 text-rose-500 disabled:bg-slate-50 disabled:text-slate-300"><Trash2 className="h-3.5 w-3.5" /></button>
                  </div>
                </article>
              );
            })}
        ...
        <section className="space-y-5 xl:order-1 xl:col-span-5">
          ...
          {editing ? (
            <form onSubmit={saveEditing} className="space-y-4 rounded-2xl bg-slate-50 p-5 sm:p-6">
              ...
              <label className="block space-y-1"><span className="text-[10px] font-bold uppercase text-slate-500">Dạng trang</span><select value={editing.kind || 'article'} onChange={event => { const kind = event.target.value as NonNullable<PortfolioNavigation['kind']>; const firstProject = projects[0]; const firstCourse = courses[0]; const firstPost = posts[0]; setEditing({ ...editing, kind, contentId: kind === 'project' ? firstProject?.id : kind === 'course' ? firstCourse?.id : kind === 'article' ? firstPost?.id : undefined, link: kind === 'project' ? `/?portfolio=true&project=${firstProject?.id || ''}` : kind === 'course' ? `/?portfolio=true&course=${firstCourse?.id || ''}` : kind === 'article' ? `/?portfolio=true&post=${firstPost?.id || ''}` : 'https://' }); }} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-xs font-semibold outline-none focus:border-brand"><option value="article">Trang bài viết</option><option value="course">Trang khóa học / học online</option><option value="project">Trang dự án Design</option><option value="external">Nhập link liên kết</option></select><span className="block text-[9px] leading-4 text-slate-400">Mục cuộn trang chủ là menu hệ thống có sẵn và không thể tạo thêm, sửa hoặc xóa.</span></label>

              {editing.kind === 'project' && <label className="block space-y-1"><span className="text-[10px] font-bold uppercase text-slate-500">Chọn dự án</span><select value={editing.contentId || ''} onChange={event => setEditing({ ...editing, contentId: event.target.value, link: `/?portfolio=true&project=${event.target.value}` })} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-xs font-semibold"><option value="">Chọn dự án...</option>{projects.map(project => <option key={project.id} value={project.id}>{project.title}</option>)}</select></label>}
              {editing.kind === 'course' && <label className="block space-y-1"><span className="text-[10px] font-bold uppercase text-slate-500">Chọn khóa học</span><select value={editing.contentId || ''} onChange={event => setEditing({ ...editing, contentId: event.target.value, link: `/?portfolio=true&course=${event.target.value}` })} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-xs font-semibold"><option value="">Chọn khóa học...</option>{courses.map(course => <option key={course.id} value={course.id}>{course.title}</option>)}</select></label>}
              {editing.kind === 'article' && <label className="block space-y-1"><span className="text-[10px] font-bold uppercase text-slate-500">Chọn bài viết</span><select value={editing.contentId || ''} onChange={event => setEditing({ ...editing, contentId: event.target.value, link: `/?portfolio=true&post=${event.target.value}` })} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-xs font-semibold"><option value="">Chọn bài viết...</option>{posts.map(post => <option key={post.id} value={post.id}>{post.title}</option>)}</select></label>}
              {editing.kind === 'external' && <label className="block space-y-1"><span className="text-[10px] font-bold uppercase text-slate-500">Link liên kết</span><input required value={editing.link} onChange={event => setEditing({ ...editing, link: event.target.value })} placeholder="https://example.com" className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-xs font-semibold outline-none focus:border-brand" /></label>}

              <div className="grid gap-3 sm:grid-cols-2"><label className="space-y-1"><span className="text-[10px] font-bold uppercase text-slate-500">Cấp menu</span><select value={editing.parentId ? 'child' : 'root'} onChange={event => setEditing({ ...editing, parentId: event.target.value === 'child' ? items.find(item => !item.parentId)?.id || null : null })} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-xs font-semibold"><option value="root">Menu cha / cấp chính</option><option value="child">Menu con</option></select></label><label className="space-y-1"><span className="text-[10px] font-bold uppercase text-slate-500">Menu cha</span><select disabled={!editing.parentId} value={editing.parentId || ''} onChange={event => setEditing({ ...editing, parentId: event.target.value || null, sortOrder: items.filter(item => item.parentId === (event.target.value || null)).length + 1 })} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-xs font-semibold disabled:bg-slate-100 disabled:text-slate-400"><option value="">Không có</option>{items.filter(item => item.id !== editing.id && !item.parentId).map(item => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label></div>
              ...
              <div className="grid grid-cols-2 gap-3"><label className="flex items-center justify-between rounded-xl bg-white p-3 text-[11px] font-bold text-slate-700">Hiển thị <input type="checkbox" checked={editing.visible} onChange={event => setEditing({ ...editing, visible: event.target.checked })} /></label><label className="flex items-center justify-between rounded-xl bg-white p-3 text-[11px] font-bold text-slate-700">Nút nổi bật <input type="checkbox" checked={!!editing.highlight} onChange={event => setEditing({ ...editing, highlight: event.target.checked })} /></label></div>
              {error && <p className="rounded-xl bg-rose-50 p-3 text-[11px] font-semibold text-rose-600">{error}</p>}
              <div className="flex gap-2"><button type="button" onClick={() => { setEditing(createNavigationDraft(items)); setError(''); }} className="flex-1 rounded-xl bg-white py-2.5 text-xs font-bold text-slate-600">Làm mới</button><button type="submit" className="flex-1 rounded-xl bg-brand py-2.5 text-xs font-bold text-white">Lưu mục menu</button></div>
            </form>
