    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl bg-slate-50 p-3">
        <div className="flex items-center gap-2 text-[11px] font-bold">
          <span className={`h-2 w-2 rounded-full ${saveState === 'local' ? 'bg-amber-500' : saveState === 'saving' ? 'animate-pulse bg-brand' : 'bg-brand'}`} />
      ...
      <div className="grid grid-cols-1 gap-7 xl:grid-cols-12">
        ...
        <section className="space-y-5 xl:order-1 xl:col-span-5">
          <div className="rounded-2xl bg-slate-950 p-4 text-white">
            ...
            {previewDevice === 'desktop' ? <div className="flex flex-wrap items-center gap-1 rounded-xl bg-white p-2">{previewRoots.map(item => { const Icon = iconMap[item.icon] || LinkIcon; const children = visiblePreviewItems.filter(child => child.parentId === item.id); return <span key={item.id} className={`inline-flex items-center gap-1 rounded-lg px-2.5 py-2 text-[10px] font-bold ${item.highlight ? 'bg-brand text-white' : 'text-slate-700'}`}><Icon className="h-3 w-3" />{item.label}{children.length > 0 && <ChevronDown className="h-3 w-3" />}</span>; })}{!previewRoots.length && <span className="p-2 text-[10px] text-slate-400">Không có menu hiển thị</span>}</div> : <div className="mx-auto max-w-[260px] space-y-1 rounded-2xl bg-white p-3">{previewMobileItems.map(item => { const Icon = iconMap[item.icon] || LinkIcon; return <div key={item.id} className={`flex items-center gap-2 rounded-lg px-3 py-2 text-[10px] font-bold ${item.parentId ? 'ml-4 text-slate-500' : 'text-slate-700'} ${item.highlight ? 'bg-brand text-white' : ''}`}><Icon className="h-3 w-3" />{item.label}</div>; })}{!previewMobileItems.length && <span className="block p-2 text-center text-[10px] text-slate-400">Không có menu hiển thị</span>}</div>}
            <button type="button" onClick={() => window.open('/?portfolio=true', '_blank', 'noopener,noreferrer')} className="mt-4 inline-flex items-center gap-1.5 text-[10px] font-bold text-brand">Mở trang Portfolio <ExternalLink className="h-3 w-3" /></button>
          </div>
