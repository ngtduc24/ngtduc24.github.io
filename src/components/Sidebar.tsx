  ...
  const renderItem = (item: { id: string; label: string; icon: any }) => {
    ...
    return (
      <a
        ...
        className={`group relative flex h-11 w-full items-center rounded-xl transition-all duration-200 cursor-pointer ${sidebarOpen ? 'gap-3 px-3' : 'justify-center px-0'} ${active ? 'bg-[#00a65a]/20 text-white shadow-[0_0_24px_rgba(0,166,90,.2)]' : 'text-slate-300 hover:bg-white/10 hover:text-white'}`}
      >
        <span className={`relative grid h-8 w-8 shrink-0 place-items-center rounded-lg transition-all ${active ? 'bg-[#00a65a] text-white shadow-lg shadow-[#00a65a]/40 animate-[pulse_2s_ease-in-out_infinite]' : 'bg-white/5 text-slate-400 group-hover:bg-white/10 group-hover:text-white'}`}>
      ...
      >
        <div className={`relative shrink-0 border-b border-white/10 ${sidebarOpen ? 'p-4' : 'px-3 py-4'}`}>
          <div className={`flex items-center ${sidebarOpen ? 'gap-3' : 'justify-center'}`}>
            <div className="grid h-11 w-11 shrink-0 place-items-center overflow-hidden rounded-2xl border-2 border-white/20 bg-[#00a65a] text-sm font-black text-white shadow-lg shadow-[#00a65a]/20">
            ...
            {sidebarOpen && <div className="min-w-0 flex-1"><p className="truncate text-sm font-black text-white">{currentUser.fullName}</p><p className="truncate text-[10px] font-semibold text-[#00a65a]">{currentUser.role === 'admin' ? 'Quản trị viên' : currentUser.role === 'member' ? 'Học viên' : 'Thành viên'}</p></div>}
          ...
          {!sidebarOpen && <button type="button" onClick={() => setSidebarOpen(true)} className="absolute -right-3 top-1/2 hidden h-7 w-7 -translate-y-1/2 place-items-center rounded-full border border-white/20 bg-slate-800 text-white shadow-lg hover:bg-[#00a65a] md:grid" aria-label="Mở đầy đủ sidebar"><PanelLeftOpen className="h-3.5 w-3.5" /></button>}
        </div>
        ...
        <div className={`shrink-0 border-t border-white/10 ${sidebarOpen ? 'p-4' : 'p-3'}`}>
          ...
          >
            <span className="grid h-8 w-8 shrink-0 place-items-center overflow-hidden rounded-lg bg-white/10">
              {settings?.webAppIcon ? <img src={settings.webAppIcon} alt="Logo hệ thống" className="h-5 w-5 object-contain" /> : <Calculator className="h-4 w-4 text-[#00a65a]" aria-label="Logo hệ thống" />}
            </span>
