      ...
      >
        <div className={`relative shrink-0 border-b border-white/10 ${sidebarOpen ? 'p-4' : 'px-3 py-4'}`}>
          <div className={`flex items-center ${sidebarOpen ? 'gap-3' : 'justify-center'}`}>
            ...
            {sidebarOpen && <div className="min-w-0 flex-1"><p className="truncate text-sm font-black text-white">{currentUser.fullName}</p><p className="truncate text-[10px] font-semibold text-brand">{currentUser.role === 'admin' ? 'Quản trị viên' : 'Thành viên'}</p></div>}
