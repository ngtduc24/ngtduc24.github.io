...
export default function QualitativeAnalysis({ users = [], currentUser, onSaveUser = async () => {}, isUserAdmin = false, settings, onRefreshSettings }: Props = {}) {
  ...
  const currentCodes = codes.filter(c => c.projectId === selectedProjectId);
        ...
        for (const s of result.suggestions) {
          ...
          if (shouldCreateCode) {
            ...
            if (!codeId) {
              ...
              saveCodes([...currentCodes, newCode]);
              codeId = newCode.id;
            }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-slate-200/80 shadow-md p-4 space-y-4">
              ...
              <form onSubmit={handleCreateCode} className="space-y-2">
                <input
                  type="text"
              ...
              <div className="space-y-2 max-h-80 overflow-y-auto">
                {currentCodes.map(code => {
                  ...
                  return (
                    <div
                      key={code.id}
                      className="flex items-center justify-between p-2 rounded-xl border border-slate-100 hover:bg-slate-50 transition-all text-xs"
                    >
                      <div className="flex items-center gap-2">
