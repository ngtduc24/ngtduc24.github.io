...
import { saveJournalToFirestore, saveJournalFieldToFirestore, saveJournalTypeToFirestore } from "../lib/firebase";
                        ...
                        try {
                          ...
                          if (data.fields && Array.isArray(data.fields)) {
                            for (const f of data.fields) {
                              if (f.id && f.name) {
                                await saveJournalFieldToFirestore(f);
                                restoredFields++;
                              }
                            }
                          }
                          ...
                          if (data.types && Array.isArray(data.types)) {
                            for (const t of data.types) {
                              if (t.id && t.name) {
                                await saveJournalTypeToFirestore(t);
                                restoredTypes++;
                              }
                            }
                          }
                          ...
                          for (const j of data.journals) {
                            if (j.id && j.name) {
                              await saveJournalToFirestore(j);
                              restoredCount++;
                            }
                          }
