...
import { collection, addDoc, getDocs, getDoc, deleteDoc, doc, query, orderBy, onSnapshot, setDoc } from 'firebase/firestore';
...
export async function getStatsFromSupabase(): Promise<Record<string, number>> {
  ...
  try {
    const docRef = doc(db, STATS_TABLE, 'visit_stats');
    const docSnap = await getDoc(docRef);
    
    if (docSnap.exists()) {
      return { ...defaultStats, ...(docSnap.data().stats || {}) };
    } else {
      return defaultStats;
    }
  } catch (error) {
...
export async function incrementStatInSupabase(key: string) {
  try {
    ...
    await setDoc(doc(db, STATS_TABLE, 'visit_stats'), { stats: updated }, { merge: true });
  } catch (error) {
    console.error(`Lỗi tăng chỉ số ${key}:`, error);
  }
}

// === NOTIFICATIONS (Migrated to Firestore) ===

export async function getNotificationsFromSupabase(): Promise<AppNotification[]> {
  try {
    const q = query(collection(db, NOTIFICATIONS_TABLE), orderBy('timestamp', 'desc'));
    ...
    console.error("Lỗi lấy thông báo từ Firestore:", error);
    return [];
  }
}
...
export async function pushNotificationToSupabase(notif: Partial<AppNotification>) {
  try {
    ...
    const docRef = await addDoc(collection(db, NOTIFICATIONS_TABLE), dbData);
    return docRef.id;
  } catch (error) {
    console.error("Lỗi gửi thông báo đến Firestore:", error);
    throw error;
  }
}
...
export async function deleteNotificationFromSupabase(notifId: string) {
  try {
    await deleteDoc(doc(db, NOTIFICATIONS_TABLE, notifId));
  } catch (error) {
    console.error("Lỗi xóa thông báo từ Firestore:", error);
    throw error;
  }
}
